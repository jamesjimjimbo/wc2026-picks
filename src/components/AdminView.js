  ...
  return (
    <div className="px-4 pb-24">
      ...
      {adminTab === 'sync' && (
        <div>
          <div className="bg-surface-secondary rounded-xl p-4 mb-4">
            <p className="text-xs font-bold text-text-primary mb-2">API-Football Sync</p>
            <p className="text-[11px] text-text-muted mb-4">
              Pull odds and results from API-Football. This runs automatically every 15 minutes via cron, 
              or you can trigger it manually here. Requires the API_FOOTBALL_KEY env variable and a paid plan.
            </p>
            <button
              onClick={handleSync}
              disabled={syncing}
            ...
            {syncStatus && (
              <div className={`mt-3 rounded-lg px-3 py-2 text-xs ${
                syncStatus.error ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-700'
              }`}>
                {syncStatus.error ? (
                  <div>
                    <p className="font-bold">Error</p>
                    <p>{syncStatus.error}</p>
                    {syncStatus.hint && <p className="mt-1 text-[10px]">{syncStatus.hint}</p>}
                    {syncStatus.details && <pre className="mt-1 text-[9px] overflow-auto">{JSON.stringify(syncStatus.details, null, 2)}</pre>}
                  </div>
                ) : (
                  <div>
                    <p className="font-bold">Sync complete</p>
                    <p>Fixtures checked: {syncStatus.fixtures_checked}</p>
                    <p>Odds updated: {syncStatus.odds_updated}</p>
                    <p>Results updated: {syncStatus.results_updated}</p>
                    {syncStatus.errors?.length > 0 && (
                      <p className="text-red-500 mt-1">{syncStatus.errors.length} errors</p>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="bg-surface-secondary rounded-xl p-4">
            <p className="text-xs font-bold text-text-primary mb-2">How it works</p>
            <div className="text-[11px] text-text-muted space-y-2">
              <p>1. A cron job hits /api/sync every 15 minutes</p>
              <p>2. It fetches all WC2026 fixtures from API-Football</p>
              <p>3. Finished matches → results are saved automatically</p>
              <p>4. Upcoming matches (next 3 days) → odds are pulled and saved</p>
              <p>5. You can still manually enter/override odds and results in the Results & Odds tab</p>
            </div>
          </div>
        </div>
      )}
